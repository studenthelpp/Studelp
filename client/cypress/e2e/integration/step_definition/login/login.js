...
Then("the user should see the navigation bar", async () => {
  await cy.get(".navigation-bar").contains("Home");
  await cy.get(".navigation-bar").contains("All Users");
  await cy.get(".navigation-bar").contains("My Profile");
  await cy.get(".navigation-bar").contains("Settings");
});
